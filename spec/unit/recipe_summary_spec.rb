...
describe Handler::RecipeSummary::Report  do
    ...
    describe '#is_loaded?' do 
        it 'boolean to determine if the cookbook is loaded' do
            expect(@handler_report.is_loaded?('nginx::default')).to be true
            expect(@handler_report.is_loaded?('test::recipe')).to be false            
        end
   end
   ...
   describe '#cookbook_recipes' do
        it 'array of cookbook and recipes' do
            ...
            expect(cookbook_recipes).to eq(["apache2::default", "openldap::default", "openldap::gigantor", "openldap::one", "openldap::return"])
        end
   end
end
